...
export const leaderboardRoutes: FastifyPluginAsync = async (server: FastifyInstance) => {
   ...
   * Fetch real-time global & community leaderboard from PostgreSQL & Redis
   */
  server.get('/leaderboards/daily', async (request, reply) => {
    const query = leaderboardQuerySchema.parse(request.query);
    const dateStr = query.date || new Date().toISOString().split('T')[0];

    const entries = await leaderboardService.getGlobalLeaderboard(dateStr, query.limit, query.community);

    return reply.status(200).send({
      date: dateStr,
      community: query.community || 'Global',
